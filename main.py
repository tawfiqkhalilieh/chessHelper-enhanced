...
def update_moves():
    ...
    count = int(
        driver.find_element(By.CLASS_NAME, "selected").get_attribute("data-ply")
    )
    ...
    for i in range(1, count + 1):
        ...
        try:
            fig += (
                driver.find_element(By.CSS_SELECTOR, f'[data-ply="{i}"]')
                .find_element(By.CSS_SELECTOR, "span[data-figurine]")
                .get_attribute("data-figurine")
            )
        except Exception as exception:
            pass
        mv = (
            fig + driver.find_element(By.CSS_SELECTOR, f'[data-ply="{i}"]').text
            if "=" not in driver.find_element(By.CSS_SELECTOR, f'[data-ply="{i}"]').text
            else driver.find_element(By.CSS_SELECTOR, f'[data-ply="{i}"]').text + fig
        )
...
elo_var = tk.StringVar(value="1800")


def set_elo() -> None:
    try:
        engine.configure({"UCI_LimitStrength": True, "UCI_Elo": int(elo_var.get())})
    except Exception as exception:
        print(exception)
...
start_label = tk.Label(
    start_frame,
    text="Welcome to the Chess Move Suggester!",
    bg="#26242f",
    fg="azure",
    width=600,
)
...
update_button = tk.Button(
    main_frame, text="Update", command=update_moves, fg="azure", bg="#26242f", width=600
)
update_button.pack()

set_elo_label = tk.Label(
    main_frame, text="Set Elo", fg="azure", bg="#26242f", width=600
)

# creating a entry for input
# name using widget Entry
set_elo_label_entry = tk.Entry(
    main_frame,
...
sub_btn = tk.Button(
    main_frame, text="Chnage Elo", command=set_elo, bg="#26242f", width=600, fg="azure"
)

set_elo_label.pack()
set_elo_label_entry.pack()
sub_btn.pack()
